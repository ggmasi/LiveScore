...
export let resultCopaFeminina;

export default function TesteCopaFeminina() {
  ...
  puppeteer.launch({headless: true, args: ['--no-sandbox', '--disable-setuid-sandbox', '--user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.150 Safari/537.36']}).then(async browser => { //abrindo o browser
    ...
    await page.goto('https://ge.globo.com/futebol/copa-do-mundo-feminina/', {timeout: 120000});
    
   

    resultCopaFeminina = await page.evaluate(() => {
      const jogos = []
      
      $('.jogo__transmissao--link').each((i, item) => { //para cada jogo, pegue os nomes dos dois times
        let obj = {
    ...
    console.log(resultCopaFeminina) 
