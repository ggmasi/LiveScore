...
export let tableResultCopaFeminina;

export default function TableTesteCopaFeminina() {
  ...
  puppeteer.launch({headless: true, defaultViewport: null, args: ['--no-sandbox', '--disable-setuid-sandbox', '--user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.150 Safari/537.36']}).then(async browser => { //abrindo o browser
    ...
    await page.goto('https://ge.globo.com/futebol/copa-do-mundo-feminina/', {timeout: 30000});
    console.log("foi")

    tableResultCopaFeminina = await page.evaluate(() => {
        const tabelaNome = []
       
      

     
        $('table.tabela__equipes tr.classificacao__tabela--linha').each((i, item) => {
            let nameTable = {
    ...
    console.log(tableResultCopaFeminina) 
